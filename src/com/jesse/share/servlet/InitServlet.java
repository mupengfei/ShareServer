...
public class InitServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		try {
			...
			CronTrigger am_trigger_1 = (CronTrigger)TriggerBuilder.newTrigger().withIdentity("am_trigger_1", "group1").withSchedule(CronScheduleBuilder.cronSchedule("0 30-59/5 9,10 ? * MON-FRI")).build();
			
			JobDetail am_job_2 = JobBuilder.newJob(TaskOfGettingShare.class).withIdentity("am_job_2", "group1").build();
			CronTrigger am_trigger_2 = (CronTrigger)TriggerBuilder.newTrigger().withIdentity("am_trigger_2", "group1").withSchedule(CronScheduleBuilder.cronSchedule("0 0-29/5 10,11 ? * MON-FRI")).build();
			
			JobDetail pm_job = JobBuilder.newJob(TaskOfGettingShare.class).withIdentity("pm_job", "group1").build();
			CronTrigger pm_trigger = (CronTrigger)TriggerBuilder.newTrigger().withIdentity("pm_trigger", "group1").withSchedule(CronScheduleBuilder.cronSchedule("0 0/5 13,14 ? * MON-FRI")).build();
